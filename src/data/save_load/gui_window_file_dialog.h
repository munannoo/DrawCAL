...
GuiWindowFileDialogState InitGuiWindowFileDialog(const char *initPath)
{
    ...
    // Init window data
    state.windowBounds = Rectangle{ static_cast<float>(GetScreenWidth()/2.0f - 440.0f/2.0f), static_cast<float>(GetScreenHeight()/2.0f - 310.0f/2.0f), 440.0f, 310.0f };
    ...
    if (state->windowActive)
    {
        ...
        if (state->supportDrag)
        {
            ...
            if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
            {
                ...
                if (CheckCollisionPointRec(mousePosition, Rectangle{ state->windowBounds.x, state->windowBounds.y, (float)state->windowBounds.width, (float)GuiGetStyle(DEFAULT, TEXT_SIZE) + 8.0f }))
                {
                    state->dragMode = true;
...
#if defined(USE_CUSTOM_LISTVIEW_FILEINFO)
        state->filesListActive = GuiListViewFiles(Rectangle{ state->windowBounds.x + 8, state->windowBounds.y + 48 + 20, state->windowBounds.width - 16, state->windowBounds.height - 60 - 16 - 68 }, fileInfo, state->dirFiles.count, &state->filesListScrollIndex, &state->filesListActive, &state->itemFocused);
#else
...
static int GuiListViewFiles(Rectangle bounds, FileInfo *files, int count, int *scrollIndex, int *active, int *focus)
{
    ...
    if (useScrollBar)
    {
        Rectangle scrollBarBounds{
            ...
            static_cast<float>(GuiGetStyle(LISTVIEW, SCROLLBAR_WIDTH)),
            bounds.height - 2.0f * GuiGetStyle(DEFAULT, BORDER_WIDTH)
        };
        ...
        GuiSetStyle(SCROLLBAR, SLIDER_WIDTH, (int)sliderSize);           // Change slider size
